...
for i in range(0, 3000 + 1, per):
    ...
    sum_v = 0
    for j in range(T):
        sum_v += csv_datas_locusts[j].loc[i]['left-grass']
    avg_v = sum_v / T

    sd_v = 0
    for j in range(T):
        sd_v += (csv_datas_locusts[j].loc[i]['left-grass'] - avg_v) ** 2
    sd_v /= T
    sd_v = math.sqrt(sd_v)
    sd_v /= math.sqrt(T)
    ...
    sum_v = 0
    for j in range(T):
        sum_v += csv_datas_locusts[j].loc[i]['right-grass']
    avg_v = sum_v / T
    sd_v = 0
    for j in range(T):
        sd_v += (csv_datas_locusts[j].loc[i]['right-grass'] - avg_v) ** 2
    sd_v /= T
    sd_v = math.sqrt(sd_v)
    sd_v /= math.sqrt(T)
...
# plt.legend()
plt.show()
figure.savefig('images/compare_without_bird_grass.png')
